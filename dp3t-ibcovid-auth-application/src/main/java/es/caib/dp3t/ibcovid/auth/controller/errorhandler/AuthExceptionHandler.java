package es.caib.dp3t.ibcovid.auth.controller.errorhandler;
...
import es.caib.dp3t.ibcovid.auth.controller.model.ErrorMessageDto;
...
public class AuthExceptionHandler {
    ...
    public ResponseEntity<List<ErrorMessageDto>> handleException(final Exception ex) {
        final IBCovidAuthException exception = new IBCovidAuthException(
                ex, AuthErrorCodes.GENERAL_ERROR, "If it persists, please contact the administrator");
    ...
    @ExceptionHandler(IBCovidAuthException.class)
    public ResponseEntity<List<ErrorMessageDto>> handleIBCovidAuthException(final IBCovidAuthException ex) {
        return processExceptionAndLogError(ex);
    }

    private ResponseEntity<List<ErrorMessageDto>> processExceptionAndLogError(final IBCovidAuthException ex) {
        log.error(ex.getMessage(), ex);
        final ErrorMessageDto errorMessage = getErrorMessage(ex);
        return buildResponse(errorMessage, ex.getError().getHttpStatus());
    }

    private ErrorMessageDto getErrorMessage(final IBCovidAuthException ex) {
    ...
    private ErrorMessageDto getErrorMessage(final ObjectError error) {
        ...
        return ErrorMessageDto.builder()
                .code(AuthErrorCodes.VALIDATION_ERROR.getCode())
